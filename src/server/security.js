...
function rotateSession(token){const user=auth.requireLogin(token);const dbmod=require('./db');const fresh=dbmod.createSession_(user.email);dbmod.destroySession_(token);return{token:fresh,success:true};}
